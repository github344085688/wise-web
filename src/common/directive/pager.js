...
define(['./directives',"lodash"], function(directives,_) {
    directives.directive('pager', function () {
        return {
            restrict: "AE",
            templateUrl: 'common/directive/template/pager.html',
            replace: true,
            scope: {
                totalCount: '=',
                pageSize: '=',
            ...
            controller: function($scope){
                $scope.pager = { activedPage: $scope.currentPage? $scope.currentPage : 1};
                $scope.inputPage = 1;
                ...
                $scope.$watch("currentPage" , function(currentPage){
                    if(currentPage != $scope.pager.activedPage ) {
                        if ($scope.pager.activedPage == currentPage)
                            return ;
                        $scope.pager.activedPage = currentPage;
        ...
        function reInitialPager(scope,isLoadContent) {
            ...
            if (scope.stay) {
                ...
                if(scope.totalCountFirstInitialed) {
                    if(isLoadContent){
                      scope.loadContent({'currentPage':scope.pager.activedPage,'pageSize':scope.pageSize, 'totalCount':scope.totalCount});
                    }
                }
            }
        }
