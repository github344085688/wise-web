...
define(['angular',
    ...
    var controller = function ($scope, fileService, $state, $stateParams, lincUtil, $mdDialog, reportService, session) {
        ...
        $scope.colDefs = [
            ...
            { headerName: 'Order Type', field: 'ORDER TYPE.', sort: true },
            ...
            { headerName: 'UOM', field: 'UOM', sort: true },
        ...
        $scope.sortClick = function (index) {
            ...
            switch (sort) {
                case SORT_DEFAULT:
                    sortDefault(index);
                    shippingByReceipts([sortField], "");
                    break;
                case SORT_ASC:
                    $scope.sorts[index] = SORT_DESC;
                    shippingByReceipts([sortField], ["desc"]);
                    break;
                case SORT_DESC:
                    $scope.sorts[index] = SORT_ASC;
                    shippingByReceipts([sortField], ["asc"]);
                    break;
            }
        };
        ...
        function shippingByReceipts(fieldName, sort) {
      
            $scope.shippingReport = _.orderBy($scope.shippingReport, fieldName, sort);
        ...
        $scope.cancel = function () {
            $state.go('rc.operation.outbound.shippingReport');
        ...
        function getReportViewByFileId(fileId) {
            reportService.getReportViewByFileId(fileId).then(function (data) {
                $scope.shippingReport = data.jsonArray.slice(1);
                $scope.loadContent(1);
            }, function (error) {
                lincUtil.processErrorResponse(error);
            });
        ...
        $scope.loadContent = function (currentPage) {
            ...
            $scope.shippingReportView = $scope.shippingReport.slice((currentPage - 1) * $scope.pageSize,
                currentPage * $scope.pageSize > $scope.shippingReport.length ? $scope.shippingReport.length : currentPage * $scope.pageSize);
        };
