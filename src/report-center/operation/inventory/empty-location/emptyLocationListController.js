...
define([
    ...
    var controller = function ($scope,  $http,  lincUtil, inventoryService) {
        ...
        $scope.loadContent = function (currentPage) {
            ...
            $scope.loading = true;
            var param ={}
            param.param=$scope.search.customerId;
            param.paging = { pageNo: Number(currentPage), limit: Number($scope.pageSize) };
            inventoryService.searchEmptyLocation(param).then(function (response) {
                $scope.loading = false;
        ...
        $scope.export = function() {
            ...
            $http.post("/report-center/inventory/empty-location-report/download", exportData, {
                ...
                lincUtil.exportFile(res, "emptyLocation.xlsx");
                $scope.exporting = false;
